...
void newNickname(FILE* fp, char* nn, ImageLayer layer) {
    ...
    int len = 0;
    char pressedKey;
    printText(layer._consoleDC, 300, 450, 60, 0, RGB(0, 0, 0), TA_LEFT, ("이름을 입력하세요(8~14자의 영문만 가능합니다.)"));
    ...
    while (3) {
        ...
        if (len != 0) printText(layer._consoleDC, 300, 650, 60, 0, RGB(0, 0, 0), TA_LEFT, ("다 입력했다면 엔터를 눌러주세요"));
        ...
        if (len > 13) {
            ...
            printText(layer._consoleDC, 100, 100, 60, 0, RGB(0, 0, 0), TA_LEFT, ("14자 초과입니다."));
            while (3) {
                pressedKey = _getch();
                if (pressedKey == '\b') {
                    break;
...
void selectGender(FILE *fp) {
    ...
    imageLayer.images = images;
    imageLayer.renderAll(&imageLayer);
	printText(imageLayer._consoleDC, 300, 100, 60, 0, RGB(0, 0, 0), TA_LEFT, TEXT("성별을 선택하세요"));
    int key, select = 0;
    while(1) {
        key = getch();
...
void selectStage(struct information *data) {
    ...
    imageLayer.images = images;
    imageLayer.renderAll(&imageLayer);
    printText(imageLayer._consoleDC, 300, 100, 60, 0, RGB(0, 0, 0), TA_LEFT, TEXT("난이도를 선택하세요"));
    int key, select = 0;
    while(1) {
        key = getch();
        if (key == RIGHT) {
...
int easyMaze(Image *images) {
    ...
    images[7].isHide = 1;
    images[8].isHide = 1;
	printText(imageLayer._consoleDC, 300, 100, 60, 0, RGB(255, 255, 255), TA_LEFT, TEXT("물을 뜨러가자!"));
    imageLayer.renderAll(&imageLayer);
